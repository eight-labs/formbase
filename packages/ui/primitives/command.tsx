...
import {
  InputGroup,
  InputGroupAddon,
} from "@formbase/ui/primitives/input-group"
...
function CommandList({
  ...
  return (
    <CommandPrimitive.List
      ...
      className={cn(
        "no-scrollbar max-h-72 scroll-py-1 outline-hidden overflow-x-hidden overflow-y-auto",
        className
      )}
      {...props}
    />
...
function CommandItem({
  ...
  return (
    <CommandPrimitive.Item
      ...
      className={cn(
        "data-selected:bg-muted data-selected:text-foreground [svg]:*:data-selected:text-foreground relative flex cursor-default items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-hidden select-none [&_svg:not([class*='size-'])]:size-4 in-data-[slot=dialog-content]:rounded-lg! group/command-item data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className
      )}
      {...props}
    >
